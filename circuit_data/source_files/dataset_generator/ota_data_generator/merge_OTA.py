...
# Establishing lists of netlist components
OTA_DIR = "ota_unbiased"
BIAS_DIR = "bias"
...
# Class containing functions joining circuit pins and logging pin errors
    ...
    def LG_pins(self, lg_pins, lg_type):
        if "LG_Vbiasn2" in self.bias_ports:
            ...
            if "Vbiasp2" in lg_pins:
                ...
                return 0
        elif "LG_Vbiasn1" not in self.bias_ports:
    ...
    def BIAS_pins(self, bias_line):
        ...
        for idx, port in enumerate(words):
            ...
            if self.LG_input == port:
                self.insts.append("\nxib" + words[1] + " " + " ".join(words[2:]) + " " + words[1])
                return 1

        return 0

# Parses files in the local_generation folder to find pin connections
...
# Parses files in the bias folder to find pin connections
...
# Loops through combinations of OTA, LG, and bias netlists, writing conbined netlists to a new FULL_OTA directory 
